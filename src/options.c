...
struct options get_options(int argc, char **argv) {
    ...
    if (*argv) {
        ...
        if (*(++argv)) {
            if (argv[1])
                error_exit(3, "too many command-line arguments");
            if (strcmp(*argv, "-"))
                result.output = *argv;
        }
    }
...
unsigned parse_numeric_option_argument(char ***alp, unsigned limit) {
    ...
    if (*error)
        error_exit(3, "invalid argument to option %s", option);
    if (result > limit)
        error_exit(3, "argument to option %s must be between 0 and %u", option, limit);
    return result;
}
...
const char *get_argument_for_option(char ***alp, const char **option_name) {
    ...
    if (!(result && *result))
        error_exit(3, "option %s requires an argument", option);
    if (option_name)
        *option_name = option;
    return result;
}
