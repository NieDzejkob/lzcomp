...
unsigned char *read_file_into_buffer(const char *file, unsigned short *size) {
    ...
    if (!fp)
        error_exit(1, "could not open file %s for reading", file);
    unsigned char *buf = malloc(MAX_FILE_SIZE + 1);
    int rv = fread(buf, 1, MAX_FILE_SIZE + 1, fp);
    if (file)
        fclose(fp);
    if (rv < 0)
        error_exit(1, "could not read from file %s", file);
    if (rv > MAX_FILE_SIZE)
        error_exit(1, "file %s is too big", file ? file : "<standard input>");
    *size = rv;
    return buf;
}
...
unsigned short compressed_length(const struct command *commands, unsigned short count) {
    ...
    for (current = 0; current < count; current++)
        if (commands[current].command != 7)
            total += command_size(commands[current]);
    return total;
}
