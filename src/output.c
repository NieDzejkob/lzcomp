...
void write_commands_to_textfile(const char *file, const struct command *commands, unsigned count, const unsigned char *input_stream,
    ...
    if (!fp)
        error_exit(1, "could not open file %s for writing", file);
    unsigned length = 0;
    while (count--) {
    ...
    if (fputs("\tlzend\n", fp) < 0)
        error_exit(1, "could not write terminator to compressed output");
    ...
    if (length--) {
        ...
        if (rv >= 0)
            rv = -(putc('\n', fp) == EOF);
        if (rv < 0)
            error_exit(1, "could not write padding to compressed output");
    }
    if (file)
        fclose(fp);
}
...
void write_command_to_textfile(FILE * fp, struct command command, const unsigned char *input_stream) {
    ...
    case 0:
        if ((rv = fprintf(fp, "\tlzdata")) < 0)
            break;
        for (pos = 0; pos < command.count; pos++)
            if ((rv = fprintf(fp, "%s$%02hhx", pos ? ", " : " ", input_stream[command.value + pos])) < 0)
                break;
        rv = putc('\n', fp);
        break;
    case 1:
    ...
    if (rv < 0)
        error_exit(1, "could not write command to compressed output");
}
...
void write_command_to_file(FILE * fp, struct command command, const unsigned char *input_stream) {
    ...
    if (fwrite(buf, 1, pos - buf, fp) != (unsigned) (pos - buf))
        error_exit(1, "could not write command to compressed output");
    if (command.command)
        return;
    command.count++;
    if (fwrite(input_stream + command.value, 1, command.count, fp) != command.count)
        error_exit(1, "could not write data to compressed output");
}
...
void write_raw_data_to_file(const char *file, const void *data, unsigned length) {
    ...
    while (length) {
        ...
        if (!rv)
            error_exit(1, "could not write raw data to output");
